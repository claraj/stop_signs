...
filename = 'bw_images/image-9.bmp'

"""
pre_thinned = 'out/bw_images/pre_thinned/image-9_thinned.bmp'
...
def is_there_a_stop_sign(image_bytes, height, width, row_length, chunk_image=False):
    ...
    lines, bounding_box = find_stop(image_bytes, height, width, row_length, 0, 0)   # bounding_box needs to be a list with coords [top left x, top left y, bottom right x, bottom right y]
    if bounding_box:  # if stop sign found,
        return    
    ...
    if chunk_image:
        ...
        for index, chunk in enumerate(image_chunk_iterator(image_bytes, height, width, row_length, chunk_size, chunk_overlap)):
            ...
            lines, bounding_box = find_stop(pixel_chunk, chunk_height, chunk_width, chunk_width, start_col, start_row)
...
def find_stop(image_bytes, height, width, row_length, offset_width, offset_height):  
    ...
    if result:
        ...
        stop_sign_lines, bounding_box = result

        bounding_box[0] += offset_width
        bounding_box[2] += offset_width
        
        bounding_box[1] += offset_height
        bounding_box[3] += offset_height
